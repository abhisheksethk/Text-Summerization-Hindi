...
text = open("input1.txt", encoding="utf8").read()
...
#Sentence Numerical Feature
sentenceNumeric = dict()
for sentence in sentences:
    count=1
    for word in sentence:
        if((isinstance(word,int))or (isinstance(word,float))):
            sentenceNumeric[sentence] +=count
            count=count+1


#print(sentenceNumeric)
# Normalize the value of sentence rank as 0 to 1
for sentence in sentenceNumeric:
    sentenceNumeric[sentence] = (sentenceNumeric[sentence] * (1.000000)) / max(sentenceNumeric.values())
